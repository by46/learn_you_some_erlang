...
![](images/common-pattern.png)
Switches to something more like this for the programmer:

![](images/event-handler-pattern.png)
...
![](images/event-handler-pattern-local.png)
...
handle_event(Event,State)是gen_event的核心。他和gen_server的handle_cast很像，都是异步工作的。只是他的返回值略显不同：
    • {ok, NewState}
    • {ok, NewState, hibernate}, 这将导致event manager自己进入休眠状态， 知道下一个事件
    • remove_handler
    • {swap_handler, Args, NewState, NewHandler, Args2}
